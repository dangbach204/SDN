...
def _ryu_candidates() -> list[str]:
    ...
    candidates = [primary, *fallback_urls,
...
def _ovs_block(dpid: int, port_no: int) -> tuple[bool, str]:
    cmd = ["sudo", "ovs-ofctl", "-O", "OpenFlow13", "add-flow", f"s{dpid}",
...
def _ovs_unblock(dpid: int, port_no: int) -> tuple[bool, str]:
    cmd = ["sudo", "ovs-ofctl", "-O", "OpenFlow13", "--strict",
           "del-flows", f"s{dpid}",
...
def _has_drop_rule(flow_dump: str, port_no: int) -> bool:
    for line in flow_dump.splitlines():
        if (f"priority={BLOCK_PRIORITY}" in line
...
def _read_tc_qdisc(dpid: int, port_no: int) -> tuple[bool, str]:
    ...
    res = subprocess.run(
        ["sudo", "tc", "qdisc", "show", "dev", port_name],
...
def _verify_action_effect(dpid: int, port_no: int,
    ...
    if normalized == "unblock":
        ...
        return True, f"DROP rule đã được gỡ trên in_port={port_no}"
    ...
    if normalized in {"monitor", "investigate"}:
        return True, "Action dạng quan sát, không cần xác minh"
...
async def choose_action(rec_id: int, body: ChooseBody):
    ...
    verified_ok, verification_msg = False, "Bỏ qua xác minh vì action thất bại"
    if ok:
        ...
        async with pool.acquire() as conn:
            await conn.execute(
                """UPDATE recommendations
                   SET status='applied', chosen_action=$1, applied_at=NOW()
                   WHERE id=$2""",
...
# POST port action (reset_qos / unblock)
...
async def execute_port_action(dpid: int, port_no: int, body: PortActionBody):
    ...
    if ok:
        verified_ok, verification_msg = _verify_action_effect(
            dpid, port_no, normalized_type, body.param)
...
async def _sync_port_state_after_manual_action(
        dpid: int, port_no: int, action_type: str) -> None:
    normalized = (action_type or "").strip().lower()
    pool = await get_pool()
    async with pool.acquire() as conn:
        if normalized == "reset_qos":
            await conn.execute("""
                ...
                WHERE dpid=$1 AND port_no=$2 AND status='applied'
                  AND (action_type='limit_bandwidth'
...
async def _execute_action(dpid: int, port_no: int,
    ...
    if normalized in {"qos", "limit_bandwidth"} and param_mbps > 0:
        port_name = f"s{dpid}-eth{port_no}"
        subprocess.run(f"sudo tc qdisc del dev {port_name} root 2>/dev/null",
                       shell=True)
        r = subprocess.run(
        ...
        ok, detail = await _ryu_post("/stats/flowentry/add", {
            "dpid": dpid, "priority": BLOCK_PRIORITY,
        ...
        return False, f"Ryu failed: {detail}. Fallback failed: {ovs_detail}"

    elif normalized == "unblock":
        ok, detail = await _ryu_post("/stats/flowentry/delete", {
            "dpid": dpid, "priority": BLOCK_PRIORITY,
        ...
        port_name = f"s{dpid}-eth{port_no}"
        res = subprocess.run(
            ["sudo", "tc", "qdisc", "del", "dev", port_name, "root"],
            capture_output=True, text=True)
