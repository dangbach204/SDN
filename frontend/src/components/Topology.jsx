...
export default function Topology({ portStats }) {
  ...
  function selectSwitch(id) {
    const ports = portStats
      .filter(r => Number(r.dpid) === id)
      .slice()
      .sort((a, b) => Number(a.port_no) - Number(b.port_no))
      ...
      <div className="topo-wrap">
        <svg className="topo-svg" viewBox="0 0 520 250">
          <g style={{ cursor: "pointer" }} onClick={() => setSelected({ type: "controller" })}>
            ...
            <text x="260" y="20" textAnchor="middle" fontFamily="DM Sans" fontSize="13" fontWeight="600" fill="#4a7fe0">Ryu Controller</text>
            <text x="260" y="33" textAnchor="middle" fontFamily="DM Mono" fontSize="10" fill="#9ca3af">127.0.0.1:6653</text>
          </g>
          ...
          <line x1="70" y1="118" x2="230" y2="118" stroke={linkColor(uplinkPct12)} strokeWidth={linkWidth(uplinkPct12)} />
          <text x="150" y="113" textAnchor="middle" fontFamily="DM Mono" fontSize="10" fill="#9ca3af">
            {uplinkPct12.toFixed(0)}%
          </text>

          <line x1="230" y1="118" x2="390" y2="118" stroke={linkColor(uplinkPct23)} strokeWidth={linkWidth(uplinkPct23)} />
          <text x="310" y="113" textAnchor="middle" fontFamily="DM Mono" fontSize="10" fill="#9ca3af">
              ...
              <g key={sw.id}>
                <g style={{ cursor: "pointer" }} onClick={() => selectSwitch(sw.id)}>
                  <rect
                    ...
                    height="44"
                    rx="8"
                    fill="rgba(74,127,224,0.08)"
                    stroke="#4a7fe0"
                    strokeWidth="1.5"
                  />
                  <text x={sw.x} y="112" textAnchor="middle" fontFamily="DM Sans" fontSize="13" fontWeight="600" fill="#1a1d2e">s{sw.id}</text>
                  <text x={sw.x} y="128" textAnchor="middle" fontFamily="DM Mono" fontSize="10" fill={col}>{u.toFixed(0)}%</text>
                </g>
                ...
                {sw.hosts.map((h, hi) => {
                  ...
                  return (
                    <g key={h} style={{ cursor: "pointer" }} onClick={() => selectHost(h)}>
                      <line x1={sw.x} y1="140" x2={hx} y2={hy} stroke="#d0d4e8" strokeWidth="1.5" />
                      <circle cx={hx} cy={hy} r="14" fill="#f7f8fc" stroke="#d0d4e8" strokeWidth="1.5" />
                      <text x={hx} y={hy + 1} textAnchor="middle" dominantBaseline="middle" fontFamily="DM Mono" fontSize="9" fill="#6b7280">
                        {h}
                      </text>
                    </g>
                  )
          ...
          {selected.type === "switch" && (
            <>
              ...
              <div className="text-muted" style={{ fontSize: 13, marginBottom: 8 }}>
                Tổng BW: <strong style={{ color: "var(--blue)" }}>{(selected.total_bps / 1e6).toFixed(1)} Mbps</strong>
                {" · "}
